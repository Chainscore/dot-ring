...
def generate_bls_signature(
    ...
    keys: List[Any] | bytes | str,
    third_party_msm: bool,
) -> bytes:
    """
    ...
    if not isinstance(producer_key, bytes):
        producer_key_point = BandersnatchPoint.string_to_point(
            bytes.fromhex(producer_key)
        )
    else:
    ...
    for key in keys:
        if isinstance(key, bytes):
            ...
            point = BandersnatchPoint.string_to_point(
                bytes(key)
            )  # or take key[2:] by skipping '0x'
...
def construct_ring_root(keys: List[Any] | str | bytes, third_party_msm: bool) -> bytes:
    ...
    fxd_col_cs = (
        ...
        + bytes.fromhex(H.bls_g1_compress(fixed_cols[1].commitment))
        + bytes.fromhex(H.bls_g1_compress(fixed_cols[2].commitment))
    )
    return fxd_col_cs
...
def ring_vrf_proof(
    ...
    if isinstance(keys, list):
        ring_proof = generate_bls_signature(
            blinding_factor, producer_key, keys, third_party_msm
        )
        ...
        ring_proof = generate_bls_signature(
            blinding_factor, producer_key, B_keys, third_party_msm
        )
...
def ring_vrf_proof_verify(
    ...
    proof: bytes | str,
    alpha: bytes | str = b"",
) -> bool:  # context(add), ring_root, signature, message(alpha)
    ...
    proof_ptr = [
        ...
        H.to_scalar_int(
            ring_proof[(48 * 4) + (7 * 32) + 48 : 48 * 4 + (7 * 32) + 48 + 32]
        ),
