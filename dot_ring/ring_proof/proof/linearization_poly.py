...
class LAggPoly:
    ...
    def evaluate_polys_at_zeta(self):
        self.P_x_zeta = poly_evaluate(self.fs[0].coeffs, [self.zeta], S_PRIME)[0]
        self.P_y_zeta = poly_evaluate(self.fs[1].coeffs, [self.zeta], S_PRIME)[0]
        self.s_zeta = poly_evaluate(self.fs[2].coeffs, [self.zeta], S_PRIME)[0]
        self.b_zeta = poly_evaluate(self.wts[0].coeffs, [self.zeta], S_PRIME)[0]
        self.acc_ip_zeta = poly_evaluate(self.wts[3].coeffs, [self.zeta], S_PRIME)[0]
        self.acc_x_zeta = poly_evaluate(self.wts[1].coeffs, [self.zeta], S_PRIME)[0]
        self.acc_y_zeta = poly_evaluate(self.wts[2].coeffs, [self.zeta], S_PRIME)[0]
    ...
    def l_agg_poly(self):
        ...
        l_agg_zeta_omega = poly_evaluate(l_agg, [self.zeta_omega], S_PRIME)[0]
        return (
            self.t,
            self.zeta,
            {
