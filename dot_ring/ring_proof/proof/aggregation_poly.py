...
class AggPoly:
    ...
    def construct_proof(self):
        ...
        C_b, C_acc_ip, C_acc_x, C_acc_y = (
            self.ws[0].commitment,
            self.ws[3].commitment,
            self.ws[1].commitment,
            self.ws[2].commitment,
        )
        ...
        Proof_B_Str_rpr = (
            H.bls_g1_compress(C_b)
            + H.bls_g1_compress(C_acc_ip)
            + H.bls_g1_compress(C_acc_x)
            + H.bls_g1_compress(C_acc_y)
            + H.to_bytes(self.P_x_zeta)
            + H.to_bytes(self.P_y_zeta)
            + H.to_bytes(self.s_zeta)
            + H.to_bytes(self.b_zeta)
            + H.to_bytes(self.acc_ip_zeta)
            + H.to_bytes(self.acc_x_zeta)
            + H.to_bytes(self.acc_y_zeta)
            + H.bls_g1_compress(self.C_q)
            + H.to_bytes(self.l_agg_at_zeta_omega)
            + H.bls_g1_compress(phi_z)
            + H.bls_g1_compress(phi_zw)
        )
        return (
            self.cf_vectors,
            Proof_P_rpr,
            Proof_B_Str_rpr,
        )  # , fs[0].commitment, fs[1].commitment, fs[2].commitment
