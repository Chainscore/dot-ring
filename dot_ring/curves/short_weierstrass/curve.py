...
"""Short-Weierstrass representation of Bandersnatch curve (migrated).

The implementation is copied from the legacy `dot_ring.curve.short_weierstrass` package
with imports unchanged, except it now lives under ``dot_ring.curves``.
"""
...
class ShortWeierstrassCurve:
    ...
    SEED_POINT: Optional[Tuple[int, int]] = None  # will be set at import-time
    PADDING_POINT: Optional[Tuple[int, int]] = None
    BLINDING_POINT: Optional[Tuple[int, int]] = None
    ...
    # --------------------------------------------------------------------
    # Utilities: sqrt, compression helpers
    # --------------------------------------------------------------------
