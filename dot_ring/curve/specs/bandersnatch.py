...
class BandersnatchPoint(TEAffinePoint[TECurve]):
    ...
    def msm(cls, points: list[Self], scalars: list[int]) -> Self:
        ...
        if len(points) == 2:
            ...
            return cast(Self, BandersnatchGLV.multi_scalar_mult_4(k1_1, k1_2, k2_1, k2_2, points[0], phi_P1, points[1], phi_P2))

        if len(points) == 4:
            return BandersnatchGLV.multi_scalar_mult_4(
                scalars[0], scalars[1], scalars[2], scalars[3], points[0], points[1], points[2], points[3]
            )

        if len(points) == 3:
            # Pad to 4 with identity and 0 scalar
            return BandersnatchGLV.multi_scalar_mult_4(
                scalars[0], scalars[1], scalars[2], 0, points[0], points[1], points[2], cls.identity()
            )
