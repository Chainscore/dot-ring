...
def compress_g1(params: RingProofParams, point: Any) -> bytes:
...
def decompress_g1(params: RingProofParams, data: bytes) -> Any:
    if params.pcs.commitment_size == 32:
        return params.pcs.decompress_g1(data)
    return H.bls_g1_decompress(data.hex())


def transcript_g1(params: RingProofParams, point: Any) -> Any:
    if params.pcs.commitment_size == 32:
        return params.pcs.serialize_g1_uncompressed(point)
    return params.pcs.normalize_g1(point)


def transcript_vk(params: RingProofParams, commitments: list[Any]) -> dict[str, Any]:
    if params.pcs.commitment_size == 32:
        return {
