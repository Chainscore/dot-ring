...
from dot_ring import Bandersnatch, Ed25519, P256
from dot_ring import IETF_VRF, PedersenVRF, RingVRF
...
class TestVectorGenerator:
    ...
    def _secret_from_seed(self, seed: int) -> bytes:
        ...
        return sk_int.to_bytes(32, 'little')
    
    def _get_scalar_len(self) -> int:
        """Get scalar length in bytes for the curve."""
        return (self.curve.curve.PRIME_FIELD.bit_length() + 7) // 8
    ...
    def generate_ietf_vector(
        ...
        pk = IETF_VRF[self.curve].get_public_key(sk)
        
        # Generate proof
        proof = IETF_VRF[self.curve].prove(alpha, sk, ad, salt)
        
        # Get input point (h)
        h = IETF_VRF[self.curve].cv.point.encode_to_curve(alpha, salt)
        
        # Get output hash (beta)
        beta = IETF_VRF[self.curve].proof_to_hash(proof.output_point)
        
        # Serialize challenge with proper length
        challenge_len = self.curve.curve.CHALLENGE_LENGTH
        c_bytes = Helpers.int_to_str(proof.c, self.curve.curve.ENDIAN, challenge_len)
    ...
    def generate_pedersen_vector(
        ...
        sk = self._secret_from_seed(seed)
        pk = PedersenVRF[self.curve].get_public_key(sk)
        
        # Generate proof
        proof = PedersenVRF[self.curve].prove(alpha, sk, ad, salt)
        
        # Get input point (h)
        h = PedersenVRF[self.curve].cv.point.encode_to_curve(alpha, salt)
        
        # Get output hash (beta)
        beta = PedersenVRF[self.curve].proof_to_hash(proof.output_point)
        ...
        blinding = PedersenVRF[self.curve].blinding(
    ...
    def generate_ring_vector(
        ...
        sk = self._secret_from_seed(seed)
        pk = RingVRF[self.curve].get_public_key(sk)
        ...
        for i in range(ring_size):
            if i == prover_idx:
                ...
                other_pk = RingVRF[self.curve].get_public_key(other_sk)
                ring_pks.append(other_pk)
        
        # Generate proof
        proof = RingVRF[self.curve].prove(alpha, ad, sk, pk, ring_pks)
        
        # Get input point (h)
        h = RingVRF[self.curve].cv.point.encode_to_curve(alpha, salt)
        
        # Get output hash (beta)
        beta = RingVRF[self.curve].proof_to_hash(proof.pedersen_proof.output_point)
        ...
        s_bytes = Helpers.int_to_str(proof.pedersen_proof.s, self.curve.curve.ENDIAN, scalar_len)
        sb_bytes = Helpers.int_to_str(proof.pedersen_proof.sb, self.curve.curve.ENDIAN, scalar_len)
        
        # Construct ring root
        ring_root = RingVRF[self.curve].construct_ring_root(ring_pks)
        ...
        return {
            ...
            "ad": ad.hex(),
            "h": h.point_to_string().hex(),
            "gamma": proof.pedersen_proof.output_point.point_to_string().hex(),
            "beta": beta.hex(),
            "blinding": blinding_bytes.hex(),
            "proof_pk_com": proof.pedersen_proof.blinded_pk.point_to_string().hex(),
            "proof_r": proof.pedersen_proof.result_point.point_to_string().hex(),
            "proof_ok": proof.pedersen_proof.ok.point_to_string().hex(),
            "proof_s": s_bytes.hex(),
            "proof_sb": sb_bytes.hex(),
            "ring_pks": ring_pks_concat.hex(),
            "ring_pks_com": ring_root.to_bytes().hex(),
            "ring_proof": proof.to_bytes().hex(),
...
def generate_ietf_vectors(generator: TestVectorGenerator) -> List[Dict]:
    ...
    for i, (seed, alpha, ad, desc) in enumerate(test_cases, 1):
...
def generate_ietf_edge_case_vectors(generator: TestVectorGenerator) -> List[Dict]:
    ...
    edge_cases = [
        ...
        (16, "Hello, 世界!".encode('utf-8'), b"", "UTF-8 encoded string"),
...
def generate_ring_vectors(generator: TestVectorGenerator) -> List[Dict]:
    ...
    test_cases = [
        ...
        (6, bytes.fromhex("42616e646572736e6174636820766563746f72"), bytes.fromhex("1f42"), 3, "different key same input"),
    ]
    
    for i, (seed, alpha, ad, idx, desc) in enumerate(test_cases, 1):
        comment = f"{suite} - vector-{i}"
        try:
...
def generate_negative_test_vectors() -> Dict[str, List[Dict]]:
    ...
    wrong_pk = IETF_VRF[Bandersnatch].get_public_key(wrong_sk)
...
def save_vectors(vectors: List[Dict], filename: str):
