...
def test_ietf_ark_bandersnatch():
    ...
    with open(data_dir) as f:
        ...
        for _j, vector in enumerate(data):
            secret_scalar, alpha, additional_data = (
    ...
    with open(data_dir) as f:
        ...
        for _j, vector in enumerate(data):
            ...
            assert proof.s.to_bytes(
                (Bandersnatch.curve.PRIME_FIELD.bit_length() + 7) // 8, Bandersnatch.curve.ENDIAN
            ) == bytes.fromhex(vector["proof_s"])
            assert proof.sb.to_bytes(
                (Bandersnatch.curve.PRIME_FIELD.bit_length() + 7) // 8, Bandersnatch.curve.ENDIAN
            ) == bytes.fromhex(vector["proof_sb"])
            assert (
                PedersenVRF[Bandersnatch].ecvrf_proof_to_hash(proof.output_point.point_to_string()).hex()
                == vector["beta"]
            )
