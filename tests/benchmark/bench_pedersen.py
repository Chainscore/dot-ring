#!/usr/bin/env python3
"""Benchmark fresh Pedersen VRF proof and verification timings."""
...
def benchmark_once(sample_index: int) -> Timing:
    ...
    proof = PedersenVRF[Bandersnatch].prove(alpha, secret_key, ad, salt)
    prove_ms = (time.perf_counter() - start) * 1000

    start = time.perf_counter()
    verified = proof.verify(alpha, ad, salt)
    verify_ms = (time.perf_counter() - start) * 1000

    if not verified:
        raise AssertionError("Pedersen VRF verification failed")
...
def run(samples: int) -> None:
    ...
    print(f"Pedersen VRF benchmark: samples={samples}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark fresh Pedersen VRF prove and verify timings.")
