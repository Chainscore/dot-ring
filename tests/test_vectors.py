...
class TestIETFVRF:
    ...
    def verify_ietf_vector(vector: Dict[str, Any], curve) -> None:
        ...
        assert gamma_bytes == expected_gamma, \
...
class TestPedersenVRF:
    ...
    def verify_pedersen_vector(vector: Dict[str, Any], curve) -> None:
        ...
        assert proof.blinded_pk.point_to_string() == expected_pk_com, \
            f"pk_com mismatch"
        
        assert proof.result_point.point_to_string() == expected_r, \
            f"R mismatch"
        
        assert proof.ok.point_to_string() == expected_ok, \
            f"Ok mismatch"
