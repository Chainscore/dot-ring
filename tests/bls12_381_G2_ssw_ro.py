    ...
    def test_sswu_hash2_curve(self):
        ...
        for i, vector in enumerate(self.test_vectors['vectors']):
            with self.subTest(f"Test Vector {i + 1}"):
                ...
                expected_u1 = FieldElement(
                    self.hex_to_field_element(expected_u[1]['re'], p),
                    self.hex_to_field_element(expected_u[1]['im'], p),
                    p
                )
                ...
                computed_Q1 = result["Q1"]
                computed_u0, computed_u1 = result["u"]
                ...
                expected_Q1_x = FieldElement(
                    self.hex_to_field_element(expected_Q1['x']['re'], p),
                    self.hex_to_field_element(expected_Q1['x']['im'], p),
                    p
                )
                expected_Q1_y = FieldElement(
                    self.hex_to_field_element(expected_Q1['y']['re'], p),
                    self.hex_to_field_element(expected_Q1['y']['im'], p),
                    p
                )
                ...
                print(f"Expected u1: {expected_u1}")
                print(f"Computed u1: {computed_u1}")
                
                # # Assert u values match
                self.assertEqual(computed_u0, expected_u0, "u0 does not match")
                self.assertEqual(computed_u1, expected_u1, "u1 does not match")
                ...
                # Compare Q1
                print("\n--- Q1 ---")
                print(f"Expected x: {expected_Q1_x}")
                print(f"Computed x: {computed_Q1.x}")
                print(f"Expected y: {expected_Q1_y}")
                print(f"Computed y: {computed_Q1.y}")
                
                # Assert Q1 matches
                self.assertEqual(computed_Q1.x, expected_Q1_x, "Q1.x does not match")
                self.assertEqual(computed_Q1.y, expected_Q1_y, "Q1.y does not match")
